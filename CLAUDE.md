...
- **L のバッテリー過放電（2026-08-15 判明・要注意）**
  - L のバッテリーが過放電状態（~1.86V）。LiPo 最低動作電圧 3.0V を大幅に下回る。
  - バッテリー駆動では BLE 広告に必要な電流を供給できず L が R に接続できない。
  - **L が BLE 接続できない場合はまず USB 給電で確認すること**。
  - 現状は USB 給電で使用可能。バッテリーが回復しない場合は交換が必要。
