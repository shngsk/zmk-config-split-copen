...
- **左手デバイス（copen2_L）が無反応**: 未解決。
  - ✅ L ハードウェア正常（L 単体 USB キーボードとして全キー入力を実機確認）
  - ✅ L-R BLE 接続は成立している（シリアルログで確認）
  - ✅ L の GATT notify は成功（`Error notifying -128` が出なくなった）
  - ❌ **しかし PC でキーが認識されない** — R 側の処理に問題がある疑い
  - → **次のアクション**: R 側の USB シリアルログを確認する（詳細は `TODO.md`）
